defmodule DemoWeb.CoreComponents.SwiftUI do
  @moduledoc """
  ...
  of the `DemoWeb.CoreComponents` functions you should expect identical functionality for similarly named
  ...
  def table(assigns) do
    ~LVN"""
    <Table id={@id}  style="padding() frame(height: 300)">
     ...
     <TableRow
        ...
        >
          <VStack :for={col <- @col}><Button><Text>
            <%= render_slot(col, @row_item.(row)) %></Text></Button>
          </VStack>
          <Button><HStack :if={@action != []}>
